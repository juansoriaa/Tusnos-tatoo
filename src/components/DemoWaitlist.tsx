                        ...
                        <div className="p-6 md:p-8 flex-1 overflow-y-auto">
                            ...
                            <div className="mb-4">
                                ...
                                <div className="mt-8 flex flex-col gap-3">
                                    ...
                                    <button 
                                        className="w-full bg-[#0b5047] border border-[#0b5047] text-white font-label-md text-label-md py-4 flex items-center justify-center gap-2 hover:brightness-110 hover:scale-[1.02] hover:shadow-[0_0_15px_rgba(11,80,71,0.4)] active:scale-95 transition-all duration-300"
                                        onClick={() => {
                                            const phone = selectedMessage.phone || selectedMessage.name.replace(/[^0-9]/g, '');
                                            let waNumber = phone;
                                            if (waNumber && !waNumber.startsWith('549') && waNumber.length === 10) {
                                                waNumber = '549' + waNumber;
                                            } else if (!waNumber) {
                                                alert("No hay un número de teléfono válido para este mensaje.");
                                                return;
                                            }
                                            
                                            let messageText = `Hola ${selectedMessage.name}, recibí tu solicitud para `;
                                            if (selectedMessage.title === 'Idea de tatuaje' || selectedMessage.tags?.includes('Idea')) {
                                                messageText += 'tu idea de tatuaje.';
                                            } else {
                                                messageText += 'tu consulta general.';
                                            }
                                            messageText += ' ¿Cómo estás?';
