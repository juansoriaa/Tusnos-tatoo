...
code = code.replace(
    /if \(isDemoUser\) \{\s*const deletedFallbacks = JSON\.parse\(localStorage\.getItem\('deletedFallbacks'\) \|\| '\[\]'\);\s*const filteredFallback = fallback\.filter\(f => !dbPhotos\.some\(p => p\.originalFallbackId === f\.id\) && !deletedFallbacks\.includes\(f\.id\)\);\s*finalPhotos = \[\.\.\.finalPhotos, \.\.\.filteredFallback\];\s*\}/,
    ''
);
