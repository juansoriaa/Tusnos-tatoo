...
// 1. Add tiktok state
if (!content.includes('const [tiktok, setTiktok]')) {
    content = content.replace("const [facebook, setFacebook] = useState('');", "const [facebook, setFacebook] = useState('');\n    const [tiktok, setTiktok] = useState('');");
    
    // Add to demoData object
    content = content.replace("facebook: facebook,", "facebook: facebook,\n                                        tiktok: tiktok,");
    
    // Add to useEffect load
    content = content.replace("setFacebook(data.facebook || '');", "setFacebook(data.facebook || '');\n                setTiktok(data.tiktok || '');");
}

// 2. Fix empty string defaults in useEffect
content = content.replace("setName(data.displayName || '');", "setName(data.displayName || 'Victor Ink');");
content = content.replace("setBio(data.bio || '');", "setBio(data.bio || 'Especialista en realismo con 10 años de trayectoria. Mi enfoque se centra en crear piezas únicas que cuenten una historia a través del contraste y los detalles minuciosos del estilo black & grey. Cada tatuaje es una obra de arte diseñada específicamente para la anatomía y visión del cliente.');");

// 3. Update Social Links JSX
const oldSocials = `<div className="flex items-center gap-3 bg-surface-container-low p-3 border border-outline-variant/10">
                                        <span className="material-symbols-outlined text-on-surface-variant text-sm" data-icon="link">link</span>
                                    ...
                                    <div className="flex items-center gap-3 bg-surface-container-low p-3 border border-outline-variant/10">
                                        <span className="material-symbols-outlined text-on-surface-variant text-sm" data-icon="facebook">facebook</span>
...
const isValidUrl = (url, domain) => {
    return url.includes(domain) && url.length > 10;
};

const newSocials = `<div className="flex items-center gap-3 bg-surface-container-low p-3 border border-outline-variant/10 relative">
